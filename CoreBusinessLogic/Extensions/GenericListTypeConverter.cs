...
namespace SCMS.CoreBusinessLogic
{
    ...
    {
        protected readonly TypeConverter m_TypeConverter;

        public GenericListTypeConverter()
        {
            m_TypeConverter = TypeDescriptor.GetConverter(typeof(T));
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            if (value is string)
            {
                ...
                {
                    object item = m_TypeConverter.ConvertFromInvariantString(s);
                    if (item != null)
                    {
                        result.Add((T)item);
                    }
